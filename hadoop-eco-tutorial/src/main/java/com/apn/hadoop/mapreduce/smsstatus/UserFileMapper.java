...
public class UserFileMapper extends Mapper<LongWritable, Text, Text, Text> {
	private static final String FILE_TAG = "CD~";
	...
	protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
		...
		String tagCustName = MessageFormat.format("{0}{1}", FILE_TAG, StringUtils.trimToEmpty(values.get(1)));
		context.write(new Text(mobileNum), new Text(tagCustName));
	}
